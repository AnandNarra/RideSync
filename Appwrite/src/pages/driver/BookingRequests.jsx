...
import { Users, MapPin, Clock, Calendar, Check, X, User, Phone, Mail, FilterX, MessageSquare, ChevronRight } from 'lucide-react';
...
const BookingRequests = () => {
    ...
    const acceptedRequests = requestsData?.data?.filter(r => r.status === 'accepted') || [];
            ...
            <div className="max-w-5xl mx-auto">
                ...
                {isLoading ? (
                    ...
                    <>
                        ...
                        <div className="mb-10">
                            <h2 className="text-sm font-black text-gray-400 uppercase tracking-widest mb-4">Pending Requests ({pendingRequests.length})</h2>
                            {pendingRequests.length === 0 ? (
                                <div className="max-w-md mx-auto text-center py-12 bg-white rounded-xl shadow-sm border border-gray-200">
                                    <div className="w-16 h-16 bg-gray-50 rounded-full flex items-center justify-center mx-auto mb-4">
                                        <Users size={28} className="text-gray-300" />
                                    </div>
                                    <h3 className="text-base font-bold text-gray-900">No pending requests</h3>
                                    <p className="text-gray-500 mt-1 px-8 text-sm">When passengers book your rides, their requests will appear here for your approval.</p>
                                </div>
                            ) : (
                                <div className="grid md:grid-cols-2 gap-6">
                        ...
                        {acceptedRequests.length > 0 && (
                            <div>
                                <h2 className="text-sm font-black text-gray-400 uppercase tracking-widest mb-4">Accepted Passengers ({acceptedRequests.length})</h2>
                                <div className="grid md:grid-cols-2 gap-6">
                                    {acceptedRequests.map((request) => (
                                        <div key={request._id} className="bg-white rounded-xl border border-green-100 shadow-sm overflow-hidden flex flex-col border-l-4 border-l-green-500">
                                            <div className="p-6 flex-1">
                                                <div className="flex justify-between items-start mb-4">
                                                    <div className="flex items-center gap-4">
                                                        ...
                                                        <div>
                                                            <h3 className="text-sm font-bold text-gray-900">{request.passengerId?.fullName}</h3>
                                                            <div className="flex items-center gap-2 mt-1">
                                                                <span className="px-2 py-0.5 rounded-full bg-green-50 border border-green-100 text-green-600 text-[10px] font-bold uppercase">Accepted</span>
                                                                <span className="text-[10px] font-medium text-gray-400">{request.seatsRequested} seat{request.seatsRequested > 1 ? 's' : ''}</span>
                                                            </div>
                                                        </div>
                                                    </div>
                                                    <div className="text-right">
                                                        <p className="text-[9px] font-bold text-gray-400 uppercase tracking-wider">Earnings</p>
                                                        <p className="text-lg font-bold text-gray-900">₹{request.rideId.pricePerSeat * request.seatsRequested}</p>
                                                    </div>
                                                </div>

                                                <div className="flex items-center gap-3 bg-gray-50/50 p-3 rounded-xl border border-gray-100 mb-4">
                                                ...
                                                <button
                                                    ...
                                                    className="w-full flex items-center justify-center gap-2 px-6 py-2.5 bg-gray-900 text-white rounded-lg font-bold text-[10px] uppercase tracking-wider hover:bg-black transition-all"
                                                >
                                            ...
                                            <div className="bg-gray-50 px-4 py-2.5 border-t border-gray-100 flex items-center justify-around">
                                                <div className="flex items-center gap-2 text-[10px] font-bold text-gray-400">
                                                    <Phone size={10} className="text-gray-300" />
                                                    <span>{request.passengerId?.phoneNumber || "N/A"}</span>
                                                </div>
